...
import customview.FullyLinearLayoutManager;
 ...
 */
public class TranslucentActivity extends BaseActivity implements NestedScrollView.OnScrollChangeListener, ZdViewPagerIndicator.onTabItemClickListener {
    ...
    protected void initContentView() {
        ...
        FullyLinearLayoutManager mLayoutManager = new FullyLinearLayoutManager(this, LinearLayoutManager.VERTICAL, false);
...
//        ToastUtils.showToast(this, "当前界面" + mCurrPage + "..高度.." + scrollY);
        ...
        if (isTopHasTab) {//防止 位置不精确
            cacheListPostion.put(mCurrPage,scrollY);
        }
        Logger.d(TAG, "onScrollChange.当前滚动...." + mCurrPage + ".......scrollY......" + scrollY);
        //TODO  滚动监听  视差效果
    ...
    @Override
    public void onItemClick(int mCurrPostion) {
        //小优化 当前A 页面  点击A不变化
        if (mCurrPage == mCurrPostion) return;
        setShowData(mCurrPostion);
//        //显示列表内容
//        //点击滚动到顶部   如何在顶部  那么不执行
    ...
    private void sCrollToPostion(int mCurrPostion) {
        if (null != cacheListPostion && mCurrPage <= 2 && cacheListPostion.size() > 0) {
            // 那么滑到第一个
            int result = cacheListPostion.get(mCurrPage);
            mNestedScrollView.scrollTo(0, result);

            Logger.d(TAG, "onScrollChange....." + mCurrPostion + ".......scrollY......" + cacheListPostion.get(mCurrPage));
        }
    }
